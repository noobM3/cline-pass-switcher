...
| `WARMUP_ENABLED` / `WARMUP_TIME` / `WARMUP_TZ_OFFSET` | 每日定时唤醒额度窗口：开关 / 时刻 / 时区偏移，默认 `true` / `06:00` / `+8` |
| `WARMUP_MODEL` / `WARMUP_UPSTREAM` / `WARMUP_CATCHUP_MINUTES` | 唤醒用的模型 / 钉住渠道（空=跟随控制台配置）/ 重启补发宽限期（分钟） |
...
上游的额度按窗口计时，窗口从**当天第一条请求**开始算。每天固定时刻自动发一条 `max_tokens=256` 的小请求，就能让窗口提前起跑，白天实际使用时额度更完整。

- 时刻由 `WARMUP_TIME`（默认 `06:00`）+ `WARMUP_TZ_OFFSET`（默认 `+8`，即北京时间）决定，按固定时区偏移换算，不受宿主机时区影响。
- 走的是与控制台「发送测试」完全相同的链路（`POST /api/test` 的内部实现），因此**完整享受多上游优先级故障转移与渠道排除**；结果同时写入请求历史（带 `warmup` 标记）与 `metadata.json` 的 `warmup` 字段。
- 进程在计划时刻之后才启动（部署、重启、宿主机开机）时，只要还在 `WARMUP_CATCHUP_MINUTES`（默认 120）宽限期内且当天没跑过，会在启动约 5 秒后补发一次，避免整天空转。
- 关闭：`WARMUP_ENABLED=false`。未配置任何账号密钥时自动跳过。
- **控制台右上角常驻角标**显示今天的唤醒结果，鼠标悬停可看模型、实际命中上游、耗时、触发方式与下次时刻；页面每 60 秒自动刷新一次状态（`GET /api/warmup`），所以整夜开着也能第一时间看到 6 点是否成功。角标状态：绿=今日成功、红=今日失败（提示里带错误）、橙=今日未触发（当时服务没运行且已过补发宽限期）或已关闭、蓝=今日待触发。
- 多账号轮询模式下，这一次唤醒只会占用其中一个账号（按轮询顺序）；如需每个账号都唤醒，给每个账号单独部署或改用多个实例。
...
| 额度唤醒 | 每日定时自动发一条小请求触发额度窗口计时（见下文「额度窗口唤醒」），右上角角标显示今日是否成功，结果进请求历史 |
